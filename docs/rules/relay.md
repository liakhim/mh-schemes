...
- В режиме инсталляции занятые обычные controller RELAY-порты `pro` используют `RELAY-N-B` для подписи устройства, а соответствующий `RELAY-N-A` отображается красной лычкой `L`. Для тупого котла оба порта остаются зелёными рабочими соединениями с подписью котла и не заменяются лычкой `L`.

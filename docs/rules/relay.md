...
- Тупой котёл в любом слоте `rl6` подключается между общим `A` своей тройки и `B` текущего слота: для слотов `1..3` используется `RELAY-1-2-3-A`, для `4..6` — `RELAY-4-5-6-A`.
...
- Для тупого котла в controller RELAY-линии `pro` линия `BUS-A` подключается к `RELAY-N-B`, а `BUS-B` - к `RELAY-N-A`. Они выходят вниз от нижней грани котла, затем вправо к своим controller-портам и поднимаются в них. Для слотов `1..4` вынос `BUS-A` равен `2.5/3.5/4.5/5.5 * indent`, а `BUS-B` - `2/3/4/5 * indent`.
...
- В режиме инсталляции изогнутые провода физических RELAY-портов, занятых тупым котлом, отображаются зелёным `#2e7d32`, включая grouped A-порт и индивидуальный B-порт.

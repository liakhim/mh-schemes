...
- Объекты, не поместившиеся при повторной материализации controller/EXT-линий, возвращаются в публичный исходный список (`one_wire_modules`, `sensors` или `wired_devices`) и не теряются.
- Для однотипных объектов без `id` overflow сохраняет исходную кратность; повторная материализация не добавляет копии уже возвращённых экземпляров.
...
- Для первого устройства 1-wire линии модулей `rl6`/`rl6s` минимум учитывает нижние горизонтальные участки соседних соединений controller -> module и module -> module. `V+` проходит минимум на `1 * indent` ниже EXT-трассы, `DAT` ещё на `1 * indent` ниже, `GND` ещё на `2 * indent` ниже `V+`; при drag учитываются текущие и базовые позиции соседних модулей.
...
- Если в цепочке есть `ntc-1-wire`, для него применяется дополнительный top offset `25 * indent`.
...
- Перед добавлением `ntc-1-wire` подбор и materializer используют свободные хвостовые каналы уже существующего или уже требуемого `io4`; standalone NTC размещается после групп `0-10V`, устройств `4-20` и DI. Только ради NTC новый `io4` не добавляется.
- Обычный `ntc-sensor` в колбе и настенный `wall-ntc-sensor` с `connection_type='ntc'` используют одинаковые правила подбора: сначала доступный хвост `io4`, затем свободный слот существующего `ntc-1-wire`, затем новый `ntc-1-wire`.
- Подбор и материализация сохраняют исходный subtype NTC-датчика; `wall-ntc-sensor` не преобразуется в `ntc-sensor`.
...
- Клик по пустому NTC-слоту открывает контекстное меню с выбором обычного `ntc-sensor` в колбе или настенного `wall-ntc-sensor`; выбранный subtype добавляется непосредственно в указанный слот.
...
- Для настенного NTC на левой линии модуля используется `ntcWallSensorRightPort.svg`, на правой линии — `ntcWallSensorLeftPort.svg`.
- Занятый слот настенного NTC квадратный (`7 * indent`) и не наследует широкую геометрию датчика в колбе; SVG обрезан по полезной области и масштабируется пропорционально без внутренних боковых полей слота.
- Назначенные NTC-датчики удаляются из верхнего `sensors`.
- Пустой пользовательский `ntc-1-wire` сохраняется; автоматически удаляется только пустой модуль с id `auto-ntc-1-wire-*`.
- При удалении `ntc-1-wire` назначенные ему датчики сначала возвращаются в `scheme.sensors`.
...
- Если проводной термостат PRO с датчиком пола переводится в `controller.ext_devices`, датчик пола один раз переносится в `thermostat.additions` и считается размещённым вместе с термостатом.

## Mixing Unit Ownership

- Сервопривод `220servo` и связанный с ним датчик смесительного узла всегда назначаются одному владельцу: контроллеру `PRO` или одному модулю `rl6s`.
- Для `smart2` сервопривод размещается на `rl2s`, а владельцем его связанного датчика считается controller 1-wire линия.
- Для `ecosmart` датчик смешения назначается только по `mixing_unit_id` реально размещённого controller `220servo`; порядок остальных датчиков не влияет на выбор.
- Цифровой датчик смесительного узла занимает 1-wire линию того же владельца, на котором размещён сервопривод.
- Для NTC-датчика `PRO` допускается один общий `ntc-1-wire` на EXT-линии `rl6`/`rl6s`; NTC-датчики узлов, чьи сервоприводы находятся на controller, могут подключаться к этому модулю. Для `ecosmart` и `smart2` сохраняются их специальные правила владельца.
- Если у сервопривода нет доступной пары RELAY-S, ни он, ни связанный датчик не распределяются отдельно.
- Узел `010servo` + NTC является физическим исключением: оба устройства занимают соседние каналы одного `io4`, без `ntc-1-wire`.
- Вложенное в EXT 1-wire устройство сохраняет владельца по стабильному `module.id`, а не по позиции модуля в массиве.
- DI-модули `rl2`/`rl2s` участвуют в размещении только у `smart2`; EXT-линии участвуют только у `pro`/`ecosmart`. Несовместимые массивы сохраняются без использования как скрытая ёмкость.

...
- `pro`: wireless-линия дополнительно поднимается на `13 * indent`.
- `pro` с непустой Wi-Fi-линией: добавляется ещё `7 * indent`.
...
- Для термостата в EXT-цепочке датчик пола подключается нестандартно: `1-WIRE-GND` датчика к `1-WIRE-GND` термостата, `1-WIRE-V+` датчика к `1-WIRE-GND` термостата и `1-WIRE-DAT` датчика к `1-WIRE-DAT` термостата.

...
- Хвост и лычка рисуются на всех занятых терминалах `ecosmart`, включая `A`, `B`, `GND` и `V+`; каждая физически подключённая клемма получает собственную монтажную маркировку.
- `EXT-OUT` занят также при EXT-термостатах (`controller.ext_devices`); линия `4-20` использует `controller.devices_420`.
...
## Controller Auto Selection (`/selection`)

- Автоподбор двусторонний: на каждое изменение схемы кандидаты перебираются в порядке `CONTROLLER_TEMPLATES` (`go`/`go+` -> `smart2` -> `pro` -> `ecosmart`) и выбирается первый подходящий. Поэтому контроллер и поднимается при добавлении оборудования, и возвращается к младшему при его удалении.
- Кандидат проверяется не по «сырой» схеме, а по собранной под него: перенос выбора, нормализация модулей под правила контроллера и пересчёт обязательных модулей, затем проверка `getControllerCompatibilityIssues`. Так учитываются модули расширения, которые подбор добавит сам.
- Из пары `go`/`go+` в перебор идёт только один вариант: линии у них одинаковые, различие — радиомодуль и встроенная батарея (`go+` при беспроводных устройствах или требовании ИБП).
- Ручной выбор контроллера (`controller_selection_source = manual`) не понижается: он проверяется первым и держится, пока оборудование в него влезает. Если перестало — работает обычный перебор кандидатов.
- Если не подошёл ни один контроллер, схема остаётся на текущем, и пользователю показывается список несовместимостей.
- Модель релейных линий в подборе обязана совпадать с размещением в схеме (`balanceServos` -> `balanceRelayDevices`), иначе подбор добирает модуль, который в схеме остаётся пустым. Порядок учёта: сначала парные `double_relay` (`220servo` — только RELAY-S; `valve` — пара RELAY-S, при её отсутствии пара RELAY), затем строгие устройства линий, затем гибкие (`zoneServo`, насосы) — RELAY, с переливом на RELAY-S.
- Ёмкость PRO под клапаны без модулей: 2 пары RELAY-S + 2 пары RELAY = 4 клапана.
...
- У пустого BUS-слота `pro` левая линия подключена к `BUS-B` и выносится на `2 * indent` ниже нижней грани слота.
- Правая линия подключена к `BUS-A` и выносится на `1 * indent` ниже нижней грани слота.
- Правило не изменяет коммутацию занятого BUS-слота.
- Базовая позиция BUS-слота `pro` смещается дополнительно на `5 * indent` вниз, если занят физический RELAY-слот 4; правило действует также при наличии UPS.
- Пустой BUS-слот `pro` располагается не выше нижней границы RELAY-линии плюс `3 * indent`. При наличии UPS RELAY-линия считается от нижней границы батареи; учитываются положительные пользовательские Y-offsets relay-слотов. Занятый BUS-слот это ограничение не использует.

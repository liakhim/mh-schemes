...
- EXT-линия `ecosmart`, содержащая проводные термостаты, дополнительно смещается на `20 * indent` левее и на `20 * indent` выше базовой позиции этой линии.
...
- В режиме инсталляции занятый вход `DI-IN-N` получает синюю изогнутую линию `#1565c0`; хвосты и подписи общих клемм `DI-1-2-3-COM` и `DI-IN-4-5-6-COM` не отображаются.
- Для определения физической занятости `di6.channel_devices` имеет приоритет над legacy `di6.di_devices`, а незаполненные индексы дополняются из legacy-линии.
...
- Свободные хвостовые каналы существующего или уже требуемого `io4` могут занимать standalone sensors с `connection_type='ntc'`; они размещаются после `0-10V`, `4-20` и DI и учитываются до расчёта потребности `ntc-1-wire`.
...
- В режиме инсталляции отдельные A-порты RL2 `RELAY-1-A`/`RELAY-2-A` и grouped A-порт RL2S `RELAY-S-1-2-A` получают красную лычку `L`; `220servo`/`valve` с `double_relay` занимает и подписывает оба физических B-порта.
- Если каналы RL2 заняты насосами, каждый занятый `RELAY-N-A` показывает отдельную красную лычку `L`, а соответствующий `RELAY-N-B` — отдельный красный провод с подписью насоса.

...
- Прямые стены: привязка направления к углу 15° (тумблер «Угол») + привязка концов к
  существующим точкам (замыкание комнат); снап централизован в сторе (`resolveDrawPoint`)
- Точная правка выбранной стены: ввод длины и угла, кнопка «Выпрямить»; общие углы
  «свариваются» (`weldVertices`), чтобы комната не рвалась
- «Сверху» — истинная ортографическая камера (drei `OrthographicCamera`, параллельная
  проекция → экран линейно маппится в план; надёжно для точного черчения); «3D» — перспектива

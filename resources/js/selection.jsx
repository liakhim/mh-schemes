...
const ThermostatCard = ({ template, connection, onConnectionChange, color, onColorChange, hasFloorSensor, onFloorSensorChange, onAdd, showAdd = true, addedRows = [], onRemoveRow, onRemoveRowCompletely, onAddRow, showJsonDetails = false, tutorialStep = null, onTutorialStepChange }) => {
    ...
    const selectedTemplateKey = `${connection}|${color}|${hasFloorSensor ? 'floor' : 'no-floor'}`;
    const selectedTemplateAdded = addedRows.some((row) => row.templateKey === selectedTemplateKey);
    ...
    >
                ...
                {addedRows.map((row, index) => {
                    ...
                    return (
                        <AddedDeviceLine
                            ...
                            control={(
                                <QtyStepper
                                    tutorialRef={row.templateKey === selectedTemplateKey ? counterRef : null}
                                    count={row.count}
        ...
        >
            ...
            <div ref={connectionRef}>
                    ...
                    {THERMOSTAT_CONNECTIONS.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                data-active={isActive}
                                onClick={() => {
                                    onConnectionChange(item.value);
                                    if (tutorialStep === 'configuration') onTutorialStepChange('configuration-changed');
                                }}
                                style={{
                                    padding: '9px 18px',
                                    border: `1px solid ${isActive ? '#e3e7ef' : 'transparent'}`,
                ...
                >
                    <input
                        ...
                        onChange={(event) => {
                            onFloorSensorChange(event.target.checked);
                            if (tutorialStep === 'configuration') onTutorialStepChange('configuration-changed');
                        }}
            ...
            {showAdd && (
                <button
                    ...
                    onClick={() => {
                        onAdd();
                        if (tutorialStep === 'configuration-changed') onTutorialStepChange('configuration-added');
                    }}
        ...
        <TutorialPopover
            ...
            onNext={() => onTutorialStepChange('color')}
            showMask
        ...
        />
        <TutorialPopover anchorRef={colorRef} scopeRef={tutorialScopeRef} open={tutorialStep === 'color'} onClose={() => onTutorialStepChange(null)} onBack={() => onTutorialStepChange('connection')} onNext={() => onTutorialStepChange('floor-sensor')} showMask title="Выберите цвет термостата" description="Выберите цвет корпуса термостата." />
        <TutorialPopover anchorRef={floorSensorRef} scopeRef={tutorialScopeRef} open={tutorialStep === 'floor-sensor'} onClose={() => onTutorialStepChange(null)} onBack={() => onTutorialStepChange('color')} onNext={() => onTutorialStepChange('add')} showMask title="Нужен ли датчик пола?" description="Включите опцию, если к термостату подключается датчик температуры пола." />
        <TutorialPopover
            anchorRef={selectedTemplateAdded ? addedListRef : addRef}
            scopeRef={tutorialScopeRef}
            open={tutorialStep === 'add'}
            onClose={() => onTutorialStepChange(null)}
            onBack={() => onTutorialStepChange('floor-sensor')}
            onNext={() => onTutorialStepChange('count')}
            nextDisabled={!selectedTemplateAdded}
            showMask
            type={selectedTemplateAdded ? 'blockContent' : null}
            title={selectedTemplateAdded ? 'Термостат добавлен в систему' : 'Проверьте выбранный вариант и добавьте термостат'}
            description={selectedTemplateAdded ? 'В этом списке отображаются все добавленные варианты термостатов.' : 'Проверьте выбранные подключение, цвет и датчик пола.'}
        />
        <TutorialPopover
            ...
            onClose={() => onTutorialStepChange(null)}
            onBack={() => onTutorialStepChange(addedRows.length > 1 ? 'configuration-changed' : 'add')}
        ...
        <TutorialPopover
            ...
            onNext={() => onTutorialStepChange('configuration-changed')}
            showMask
        ...
        <TutorialPopover
            ...
            onNext={() => onTutorialStepChange('configuration-existing')}
            nextDisabled={!selectedTemplateAdded}
            showMask
            title={selectedTemplateAdded ? 'Данный вариант уже добавлен в систему' : 'Проверьте новую конфигурацию и добавьте термостат'}
            description={selectedTemplateAdded
                ? 'Измените его количество в таблице «Добавленные термостаты». Чтобы добавить новый вариант, продолжайте менять конфигурацию.'
                : 'Проверьте тип подключения, цвет и датчик пола, затем добавьте новый вариант термостата.'}
        />
        <TutorialPopover
            ...
            onClose={() => onTutorialStepChange(null)}
            onBack={() => onTutorialStepChange('configuration-changed')}
            onNext={() => onTutorialStepChange('count')}
            showMask
            type="blockContent"
            title="Термостат добавлен в систему"
            description="Новая конфигурация добавлена в таблицу «Добавленные термостаты»."
        />
        <TutorialPopover
            anchorRef={addedListRef}
            scopeRef={tutorialScopeRef}
            open={tutorialStep === 'configuration-existing'}
            onClose={() => onTutorialStepChange(null)}
            onBack={() => onTutorialStepChange('configuration-changed')}
            onNext={() => onTutorialStepChange('count')}
            showMask
            type="blockContent"
            title="Данный вариант находится в таблице"
            description="Используйте его счётчик, чтобы изменить количество термостатов этой конфигурации."
        />
    </div>
    );
};
...
const SectionEquipmentCard = ({
                        ...
                        control={(
                            <QtyStepper
                                tutorialRef={(counterTutorialTemplateKey
                                    ? row.templateKey === counterTutorialTemplateKey
                                    : counterTutorialLabel
                                        ? row.label === counterTutorialLabel
                                        : index === 0) ? counterTutorialRef : null}
                                count={row.count}
                                dense={addedDense}
    ...
    const contentMaxWidth = half ? 300 : 620;
...
const SegmentedField = ({
    ...
    <div ref={fieldRef} className={className} style={{ alignSelf: 'flex-start', width: 'fit-content', maxWidth: '100%' }}>
...
const MixingUnitCard = ({ template, connectionMode, onConnectionModeChange, servo, onServoChange, sensor, onSensorChange, onAdd, addedRows = [], onAddUnit, onRemoveUnit, onRemoveRow, wifiMixingLimitReached = false, showJsonDetails = false, tutorialStep = null, onTutorialStepChange }) => {
    ...
    const mixingSettingsRef = useRef(null);
    const addRef = useRef(null);
    const addedListRef = useRef(null);
    const counterRef = useRef(null);
    ...
    <TutorialPopover
        ...
        open={tutorialStep === 'connection'}
        onClose={() => onTutorialStepChange(null)}
        onNext={() => onTutorialStepChange('servo')}
        showMask
    ...
    <TutorialPopover
        ...
        onNext={() => onTutorialStepChange('sensor')}
        showMask
    ...
    <TutorialPopover
        ...
        onClose={() => onTutorialStepChange(null)}
        onBack={() => onTutorialStepChange('servo')}
        onNext={() => onTutorialStepChange('add')}
        showMask
    ...
    <TutorialPopover
        ...
        onClose={() => onTutorialStepChange(null)}
        onBack={() => onTutorialStepChange(addedRows.length > 1 ? 'configuration-add' : 'add')}
    ...
    <TutorialPopover
        ...
        open={tutorialStep === 'configuration'}
        onClose={() => onTutorialStepChange(null)}
        onBack={() => onTutorialStepChange('add-more')}
        onNext={() => onTutorialStepChange('configuration-add')}
        showMask
    ...
    <TutorialPopover
        ...
        open={tutorialStep === 'configuration-existing'}
        onClose={() => onTutorialStepChange(null)}
        onBack={() => onTutorialStepChange('configuration-add')}
        onNext={() => onTutorialStepChange('add-more')}
        showMask
        type="blockContent"
        title="Данный вариант находится в таблице"
        description="Используйте его счётчик, чтобы изменить количество смесительных узлов этой конфигурации."
    />
    </div>
    );
};
...
const PumpCard = ({ template, connectionMode, onConnectionModeChange, pumpType, onPumpTypeChange, onAdd, addedRows = [], onAddUnit, onRemoveUnit, onRemoveRow, wifiPumpLimitReached = false, showJsonDetails = false, tutorialStep = null, onTutorialStepChange }) => {
    ...
    const pumpSettingsRef = useRef(null);
    const addRef = useRef(null);
    const addedListRef = useRef(null);
    const counterRef = useRef(null);
    const selectedTemplateAdded = addedRows.some((row) => row.label === template.label);
    ...
    <TutorialPopover
        ...
        open={tutorialStep === 'connection'}
        onClose={() => onTutorialStepChange(null)}
        onNext={() => onTutorialStepChange('type')}
        showMask
    ...
    <TutorialPopover
        ...
        type="blockContent"
        title="Данный вариант находится в таблице"
        description="Используйте его счётчик, чтобы изменить количество насосов этой конфигурации."
    />
    </div>
    );
};
...
const ZoneCard = ({ template, connectionMode, onConnectionModeChange, onAdd, addedRows = [], onAddUnit, onRemoveUnit, onRemoveRow, wifiRelayLimitReached = false, showJsonDetails = false, tutorialStep = null, onTutorialStepChange, isOtherEquipment = false }) => {
    ...
    <div ref={tutorialScopeRef} style={{ position: 'relative', flex: '1 1 100%', minWidth: 0 }}>
    <SectionEquipmentCard
        image={isOtherEquipment ? OTHER_EQUIP_BACKGROUND_PATH : ZONES_BACKGROUND_PATH}
        backgroundWidth={isOtherEquipment ? 520 : 560}
        backgroundColor={isOtherEquipment ? CARD_PHOTO_TAIL_COLOR.otherRoom : CARD_PHOTO_TAIL_COLOR.zonesRoom}
        title={template.label}
        headerAction={(
            <button
                className="selection-option-button selection-tutorial-trigger"
        ...
        description={isOtherEquipment
            ? 'Сирены, реле и другие устройства, которыми контроллер управляет через релейный выход. Каждое занимает один релейный порт.'
            : 'Определите, на сколько зон будет разделена система, чтобы эффективно управлять оборудованием через двухходовые сервоприводы.'}
        addedTitle={isOtherEquipment ? 'Добавленное оборудование:' : 'Добавленные зоны:'}
        addedRows={addedRows}
        addedTutorialRef={addedListRef}
        counterTutorialRef={counterRef}
        counterTutorialLabel={template.label}
        ...
        qtyTestId={isOtherEquipment ? 'other-equipment-qty' : 'zone-qty'}
    ...
    <TutorialPopover
        ...
        onNext={() => onTutorialStepChange('add')}
        showMask
        title={isOtherEquipment ? 'Как подключено оборудование?' : 'Как подключён сервопривод зоны?'}
        description={isOtherEquipment
    ...
    <TutorialPopover
        ...
        open={tutorialStep === 'count'}
        onClose={() => onTutorialStepChange(null)}
        onBack={() => onTutorialStepChange(isOtherEquipment && addedRows.length > 1 ? 'configuration-add' : 'add')}
        ...
        description={isOtherEquipment
            ? 'Кнопки + и - изменяют количество одинакового оборудования. Wi-Fi устройства автоматически распределяются по свободным каналам Wi-Fi модулей.'
            : 'Кнопки + и - изменяют количество одинаковых зон. Wi-Fi сервоприводы автоматически распределяются по свободным каналам Wi-Fi модулей.'}
    />
    {isOtherEquipment && (
        <TutorialPopover
            anchorRef={connectionRef}
    ...
    {isOtherEquipment && (
        <TutorialPopover
            ...
            type="blockContent"
            title="Данный вариант находится в таблице"
            description="Используйте его счётчик, чтобы изменить количество оборудования этой конфигурации."
        />
    )}
    </div>
    );
...
const QtyStepper = ({ count, onDecrement, onIncrement, disabled = false, incrementDisabled = false, allowZero = false, dense = false, decTestId = null, incTestId = null, tutorialRef = null }) => {
    ...
    return (
        <div ref={tutorialRef} style={{ display: 'inline-flex', alignItems: 'center', gap: dense ? 3 : 4, flexShrink: 0 }}>
            <button
                type="button"
                title="Убрать одно"
...
const SelectionApp = () => {
    ...
    const [otherEquipmentTutorialStep, setOtherEquipmentTutorialStep] = useState(null);
    ...
    const [temperatureSensorTutorialStep, setTemperatureSensorTutorialStep] = useState(null);
    const temperatureSensorTutorialScopeRef = useRef(null);
    const temperatureSensorConnectionRef = useRef(null);
    const temperatureSensorPlacementRef = useRef(null);
    const temperatureSensorKindRef = useRef(null);
    ...
    const leakTutorialInitialZoneCountRef = useRef(0);
    ...
    const hasNewLeakTutorialZone = leakZoneRows.length > leakTutorialInitialZoneCountRef.current;
    ...
    // отдельных карточек. Беспроводной вариант существует только настенный.
    ...
    const temperatureSensorKindOptions = TEMPERATURE_SENSOR_KINDS;
    const selectTemperatureSensorPlacement = useCallback((placement) => {
        setWiredTemperatureSensorKey((current) => {
            const kind = current.endsWith('-ntc') ? 'ntc' : 'digital';
    ...
    return (
        ...
        >
                    ...
                    >
                        <div className="sel-boilers-header">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    if (boilerTutorialStep) {
                                        setBoilerTutorialStep(null);
                                    } else {
                                        setBoilerTutorialNextStep(null);
                                        setBoilerTutorialStep('input');
                                    }
                                }}
                            >
                                <span className="selection-tutorial-trigger-icon" aria-hidden="true">?</span>
                        ...
                        {Array.isArray(incomingScheme.boilers) && incomingScheme.boilers.length > 0 && (
                            <AddedDevicesBlock marginTop={0} tutorialRef={boilerAddedListTutorialRef}>
                        ...
                        <TutorialPopover
                            ...
                            nextDisabled={boilerResults.length === 0}
                            showMask
                        ...
                        <TutorialPopover
                            ...
                            onNext={() => setBoilerTutorialStep(boilerTutorialNextStep || 'connection')}
                            showMask
                            type="blockContent"
                        ...
                        <TutorialPopover
                            anchorRef={boilerSearchInputRef}
                            highlightRef={boilerDropdownTutorialRef}
                            highlightActive={boilerResultsVisible}
                            scopeRef={boilerTutorialScopeRef}
                            open={boilerTutorialStep === 'select'}
                            onClose={() => setBoilerTutorialStep(null)}
                            onBack={() => setBoilerTutorialStep('input')}
                            onNext={() => setBoilerTutorialStep('added-list')}
                            nextDisabled={!boilerTutorialNextStep}
                            showMask
                        ...
                        <TutorialPopover
                            ...
                            onClose={() => setBoilerTutorialStep(null)}
                            onBack={() => setBoilerTutorialStep('added-list')}
                            onNext={() => setBoilerTutorialStep('add-more')}
                            showMask
                        ...
                        <TutorialPopover
                            ...
                            onClose={() => setBoilerTutorialStep(null)}
                            onBack={() => setBoilerTutorialStep(boilerTutorialNextStep === 'add-more' ? 'added-list' : 'connection')}
                            onNext={() => setBoilerTutorialStep('search-more')}
                            showNext={false}
                            showMask
                        ...
                        <TutorialPopover
                            ...
                            onClose={() => setBoilerTutorialStep(null)}
                            onBack={() => setBoilerTutorialStep('add-more')}
                            onNext={() => setBoilerTutorialStep(null)}
                            showMask
                            title="Добавьте дополнительный котел из списка"
                            description="Или запустите поиск по другому ключевому слову (название котла или произвотеля)."
                        />
                    </div>
                </div>
            </section>
            ...
            <section style={{ flex: '1 1 100%', minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    <MixingUnitCard
                        ...
                        onTutorialStepChange={setMixingTutorialStep}
                    />
                </div>
            </section>
                ...
                <section>
                    ...
                    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                        <ZoneCard
                            ...
                            onConnectionModeChange={setZoneConnectionMode}
                            addedRows={zoneRows}
                            onAddUnit={(row) => addMixingUnit(
            ...
            <section style={{ flex: '1 1 360px', minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    <ZoneCard
                        template={otherEquipmentTemplate}
                        connectionMode={otherEquipmentConnectionMode}
                        onConnectionModeChange={setOtherEquipmentConnectionMode}
                        ...
                        wifiRelayLimitReached={wifiRelayLimitReached}
                        showJsonDetails={showJsonDetails}
                        tutorialStep={otherEquipmentTutorialStep}
                        onTutorialStepChange={setOtherEquipmentTutorialStep}
                        isOtherEquipment
                    />
                </div>
            ...
            <section>
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    <div ref={temperatureSensorTutorialScopeRef} style={{ position: 'relative', flex: '1 1 100%', minWidth: 0 }}>
                    <SectionEquipmentCard
                        image={THERMOSTAT_ROOM_IMAGE_PATH}
                        backgroundWidth={520}
                        ...
                        headerAction={(
                            <button
                                className="selection-option-button selection-tutorial-trigger"
                                type="button"
                                data-active={Boolean(temperatureSensorTutorialStep)}
                                aria-pressed={Boolean(temperatureSensorTutorialStep)}
                                aria-label={temperatureSensorTutorialStep ? 'Закрыть обучение по настройке датчиков температуры' : 'Показать подсказку по настройке датчиков температуры'}
                                title={temperatureSensorTutorialStep ? 'Закрыть обучение' : 'Как добавить датчик температуры'}
                                onClick={() => setTemperatureSensorTutorialStep(temperatureSensorTutorialStep ? null : 'connection')}
                            >
                                <span className="selection-tutorial-trigger-icon" aria-hidden="true">?</span>
                                <span className="selection-tutorial-trigger-copy">
                                    <strong>Подсказки</strong>
                                    <small><span className="selection-tutorial-trigger-status" aria-hidden="true" />{temperatureSensorTutorialStep ? 'Включены' : 'Выключены'}</small>
                                </span>
                            </button>
                        )}
                        addedTitle="Добавленные датчики температуры:"
                        addedRows={temperatureSensorRows}
                        addedTutorialRef={temperatureSensorAddedListRef}
                        counterTutorialRef={temperatureSensorCounterRef}
                        counterTutorialTemplateKey={temperatureSensorTemplate?.key}
                    ...
                    >
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end' }}>
                            <div ref={temperatureSensorConnectionRef}>
                                <ThermostatFieldLabel>Тип подключения</ThermostatFieldLabel>
                                <SegmentedToggle
                            ...
                            <div ref={temperatureSensorPlacementRef}>
                            ...
                            {!isWirelessTemperatureSensor && (
                                <div ref={temperatureSensorKindRef}>
                    ...
                    <TutorialPopover
                        anchorRef={temperatureSensorConnectionRef}
                        scopeRef={temperatureSensorTutorialScopeRef}
                        open={temperatureSensorTutorialStep === 'connection'}
                        onClose={() => setTemperatureSensorTutorialStep(null)}
                        onNext={() => setTemperatureSensorTutorialStep('placement')}
                        showMask
                    ...
                    <TutorialPopover
                        ...
                        scopeRef={temperatureSensorTutorialScopeRef}
                        open={temperatureSensorTutorialStep === 'placement'}
                        onClose={() => setTemperatureSensorTutorialStep(null)}
                        onBack={() => setTemperatureSensorTutorialStep('connection')}
                        onNext={() => setTemperatureSensorTutorialStep(isWirelessTemperatureSensor ? 'add' : 'kind')}
                        showMask
                    ...
                    <TutorialPopover
                        ...
                        scopeRef={temperatureSensorTutorialScopeRef}
                        open={temperatureSensorTutorialStep === 'kind'}
                        onClose={() => setTemperatureSensorTutorialStep(null)}
                        onBack={() => setTemperatureSensorTutorialStep('placement')}
                        onNext={() => setTemperatureSensorTutorialStep('add')}
                        showMask
                        title="Выберите тип проводного датчика"
                    ...
                    <TutorialPopover
                        ...
                        onClose={() => setTemperatureSensorTutorialStep(null)}
                        onBack={() => setTemperatureSensorTutorialStep(isWirelessTemperatureSensor ? 'placement' : 'kind')}
                        onNext={() => setTemperatureSensorTutorialStep('count')}
                        nextDisabled={!isTemperatureSensorTemplateAdded}
                        showMask
                        type={isTemperatureSensorTemplateAdded ? 'blockContent' : null}
                        title={isTemperatureSensorTemplateAdded ? 'Датчик добавлен в систему' : 'Добавьте датчик температуры'}
                        description={isTemperatureSensorTemplateAdded
                            ? 'В списке отображаются все добавленные датчики температуры.'
                            : 'Проверьте выбранную конфигурацию и добавьте датчик в систему.'}
                    />
                    <TutorialPopover
                        anchorRef={temperatureSensorCounterRef}
                        scopeRef={temperatureSensorTutorialScopeRef}
                        open={temperatureSensorTutorialStep === 'count'}
                        onClose={() => setTemperatureSensorTutorialStep(null)}
                        onBack={() => setTemperatureSensorTutorialStep('add')}
                        onNext={() => setTemperatureSensorTutorialStep(null)}
                        showMask
                        title="Изменяйте количество датчиков"
                        description="Кнопки + и - изменяют количество одинаковых датчиков температуры."
                    />
                    </div>
                </div>
            ...
            <section className="sel-leak-section">
                ...
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    ...
                    <SectionEquipmentCard
                        ...
                        headerAction={(
                            <button
                                ...
                                onClick={() => {
                                    if (leakTutorialStep) {
                                        setLeakTutorialStep(null);
                                    } else {
                                        leakTutorialInitialZoneCountRef.current = leakZoneRows.length;
                                        setLeakTutorialStep('add-zone');
                                    }
                                }}
                            >
                                <span className="selection-tutorial-trigger-icon" aria-hidden="true">?</span>
                            ...
                            {leakZoneRows.length > 0 && (
                                ...
                                {leakZoneRows.map((zone, index) => (
                                    ...
                                    >
                                        ...
                                        <QtyStepper
                                            tutorialRef={index === leakZoneRows.length - 1 ? leakZoneCounterRef : null}
                    ...
                    <TutorialPopover
                        anchorRef={hasNewLeakTutorialZone ? leakListRef : leakAddRef}
                        scopeRef={leakTutorialScopeRef}
                        open={leakTutorialStep === 'add-zone'}
                        onClose={() => setLeakTutorialStep(null)}
                        onNext={() => setLeakTutorialStep('zone-count')}
                        nextDisabled={!hasNewLeakTutorialZone}
                        showMask
                        type={hasNewLeakTutorialZone ? 'blockContent' : null}
                        title={hasNewLeakTutorialZone ? 'Группа датчиков добавлена' : 'Добавьте группу датчиков протечки'}
                        description={hasNewLeakTutorialZone
                            ? 'В каждой группе можно указать число датчиков, подключённых к одному шлейфу.'
                            : 'Все датчики одной группы образуют шлейф и занимают один дискретный вход. Для независимой зоны добавьте отдельную группу.'}
                    />
                    <TutorialPopover
                        ...
                        onClose={() => setLeakTutorialStep(null)}
                        onBack={() => setLeakTutorialStep('add-zone')}
                        onNext={() => setLeakTutorialStep('valve-count')}
                        showMask
                    ...
                    <TutorialPopover
                        ...
                        onClose={() => setLeakTutorialStep(null)}
                        onBack={() => setLeakTutorialStep('zone-count')}
                        onNext={() => setLeakTutorialStep(null)}
                        showMask

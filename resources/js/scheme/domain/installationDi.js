...
export const getRelaySupplyLabel = (portName, ownerType) => {
    ...
    const normalizedPortName = String(portName || '').trim().toUpperCase();
    const owner = canonicalDeviceType(ownerType);
    if (owner === 'pro' && /^RELAY(?:-S)?-[1-4]-A$/.test(normalizedPortName)) return 'L';
    if ((owner === 'go' || owner === 'go+') && /^RELAY-[1-4]-COM$/.test(normalizedPortName)) return 'L';
    return owner === 'rl2' && /^RELAY-[12]-A$/.test(normalizedPortName) ? 'L' : null;
};

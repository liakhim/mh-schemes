...
export const MYHEAT_SOLD_SUMMARY = {
    ...
    notSold: [
        ...
        'wired-wall-ntc — Настенный NTC-датчик температуры: доступен для добавления, но ключа нет в MYHEAT_PRICES.temperatureSensors',
...
export const SELECTION_BUTTON_MAP = [
    ...
    {
        section: '0. Общие действия',
        local: { testId: 'open-commercial-offer', description: 'Кнопка "Спецификация" под панелью подобранного контроллера — открывает модалку КП (заголовок диалога остался "Коммерческое предложение") с ценами MyHeat (источник цен — MYHEAT_PRICES)' },
    ...
    {
        section: 'Контроллер',
        local: { testId: 'controller-card-{go|go+|smart2|pro|ecosmart}', description: 'В липкой панели «Подобранный контроллер» справа видна только карточка подобранного контроллера (в конфигурации, которую закрывают и PRO, и ECOsmart — две кликабельные карточки), data-active отражает текущий выбор; ниже плитки модулей расширения panel-module-{тип}, включая panel-module-ups для smart2/pro с UPS' },
    ...
    // --- 2. Котлы ---
    {
        section: '2. Котлы',
        local: [
            { testId: null, description: 'Поле поиска котла по названию (autocomplete, placeholder "Введите название котла..."), без явной кнопки "Добавить"' },
            ...
            { testId: 'boiler-search-results', description: 'Выпадашка с найденными котлами; строка результата — boiler-search-option. Выбор котла убирает выпадашку, оставляя текст запроса и результаты; возврат в поле ввода раскрывает их снова, клик мимо блока и Escape закрывают' },
            { testId: 'boiler-search-empty', description: 'Блок "Котлы не найдены" для уже отработанного запроса' },
            { testId: 'boiler-search-clear', description: 'Крестик в поле: очищает запрос и закрывает выпадашку' },
        ],
        ...
        note: 'Совпадает по смыслу. Котлы — сторонний бренд, unitPrice для них никогда не проставляется (см. getOfferSections, секция "Котлы"). Переходник Rinnai (RINNAI_ADAPTER_PRICE) при этом продаётся отдельно. У live есть переключатель "резерв" рядом с полем — проверить, что у локальной версии он тоже есть (не заносился в выборку кнопок).\n' +
            'Поиск идёт через POST /api/integration с debounce 400 мс и стартует ТОЛЬКО от изменения текста в поле (выбор котла запрос не перезапускает). В тестах этот эндпоинт удобно подменять page.route — см. selection-boiler-search.spec.js.',
    },
    ...
    // --- 6. Дополнительные датчики ---
    {
        section: '6. Дополнительные датчики — Датчики температуры (общая карточка)',
        local: [
            ...
            { testId: 'temperature-sensor-kind-digital|-ntc', description: 'Тумблер "Цифровой / NTC", только для проводного' },
        ...
        combinations: [
            ...
            { title: 'Настенный NTC-датчик', templateKey: 'wired-wall-ntc', soldByMyHeat: false },
        ...
        soldByMyHeat: 'mixed',
        note: 'Проводные и беспроводные датчики объединены в одну карточку с тремя тумблерами — структура совпала с live-модалкой (3 select). Беспроводного датчика "в колбе" не существует. Настенный NTC-датчик доступен в подборе, но не имеет цены в MYHEAT_PRICES.temperatureSensors. Уличный датчик в этой карточке не участвует — у него отдельный тумблер outdoor-sensor-toggle.',
    },
    {
        section: '7. Защита от протечки — Контроль протечки воды',
    ...
    {
        section: '7. Защита от протечки — Запорный клапан',
    ...
    {
        section: '6. Дополнительные датчики — Датчик давления',
        local: { testId: 'add-pressure-sensor', title: 'Датчик давления' },
        live: { description: 'Счётчик "Датчик 4-20мА" под заголовком "Датчик давления"' },
        soldByMyHeat: true,
    ...
    // --- 8. Прочее (дискретные входы) ---
    {
        section: '8. Прочее — Дискретные входы',
        local: [
    ...
    {
        section: '9. Питание — Доп. блок питания',
        local: { present: false },

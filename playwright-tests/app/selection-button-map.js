...
export const SELECTION_BUTTON_MAP = [
    ...
    // --- 1. Контроллер ---
    {
        section: '1. Контроллер',
        local: { testId: 'controller-card-{go|go+|smart2|pro|ecosmart}', description: '5 кликабельных карточек ручного выбора контроллера, data-active отражает текущий выбор' },
    ...
    {
        section: '2. Котлы',
        local: { testId: null, description: 'Поле поиска котла по названию (autocomplete), без явной кнопки "Добавить"' },
    ...
    {
        section: '4. Климат — Термостаты (беспроводные и проводные)',
        local: [
            { testId: null, description: 'Кнопки цвета "Черный"/"Белый"/"Серый" (повторяются в блоках Wireless и Wired) + кнопка "Добавить термостат" на каждый блок' },
        ],
        live: {
    ...
    },
    {
        ...
        local: [
            { testId: null, description: 'В блоке wireless_devices: кнопки "Уличный датчик температуры" / "Настенный датчик температуры" (выбор типа) + кнопка "Добавить датчик"' },
        ],
        live: { description: 'Простой toggle "Беспроводной уличный датчик температуры" (вкл/выкл, без выбора типа)' },
        soldByMyHeat: true,
        note: 'Оба беспроводных датчика продаются MyHeat: wireless-outdoor 5890 ₽, wireless-wall 4190 ₽ (MYHEAT_PRICES.temperatureSensors). На live виден только toggle для уличного варианта на этой секции — настенный беспроводной датчик, скорее всего, вынесен в общую модалку "Термостат"/"Датчики температуры" на live; нужно перепроверить при написании теста, а не считать его отсутствующим.',
    },

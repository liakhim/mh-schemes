...
Публичные массивы `boilers`, `wired_devices` и `sensors` остаются каноническим источником устройств. Внутренние массивы линий контроллера и модулей (`relay_devices`, `relay_s_devices`, `bus_devices`, `di_devices`, `channel_devices`, `devices_420`) создаются для отрисовки и не заменяют публичные массивы при сохранении. При загрузке legacy-поле `controller.devices420` объединяется с `controller.devices_420` без дублирования и удаляется; legacy-назначение `connection_layout.assignments[].line: 'devices420'` нормализуется в `devices_420`.
...
- При каждом save поле полностью пересобирается из фактического materialized state; входному старому значению serializer не доверяет. Если валидных назначений нет, serializer удаляет `connection_layout`. Wi-Fi-линии не входят в `connection_layout`: их ручные owner и slot сохраняются непосредственно в `wifi_modules`.
